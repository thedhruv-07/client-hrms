...
test("createSchema accepts a minimal valid payload", () => {
  const result = createSchema.safeParse({ code: "CW-001", name: "Arun", clientId: "client-1", basicSalary: 17000 });
  assert.equal(result.success, true);
});

test("createSchema rejects a missing code", () => {
  const result = createSchema.safeParse({ name: "Arun", clientId: "client-1", basicSalary: 17000 });
  assert.equal(result.success, false);
});

test("createSchema rejects an empty code", () => {
  const result = createSchema.safeParse({ code: "", name: "Arun", clientId: "client-1", basicSalary: 17000 });
  assert.equal(result.success, false);
});

test("createSchema rejects a missing clientId", () => {
