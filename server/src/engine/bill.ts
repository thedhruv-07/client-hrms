 ...
 *
 * ESIC rounds up to the nearest whole rupee; every other money field rounds
 * to the nearest whole rupee too (no paisa anywhere in this app).
 */
...
export function calculateBill(input: BillInput): BillResult {
  ...
  const pfEmployer = round0((pfBase * 13) / 100);
  const serviceCharge = round0((total1 * 5) / 100);

  const total2 = round0(total1 + esiEmployer + pfEmployer + lwf + serviceCharge); // Taxable Amount
  const cgst = round0((total2 * 9) / 100);
  const sgst = round0((total2 * 9) / 100);
  const grandTotal = round0(total2 + cgst + sgst);

  return {
    basicWages,
    hra,
    otAmount: round0(otAmount),
    attendAward: round0(attendAward),
    incentiveAmt: round0(incentiveAmt),
    total1,
    esiEmployer,
    pfBase,
    pfEmployer,
